...
var fecha = new Date;
var fechaString = fecha.toLocaleDateString('pt-PT');
var timeString = fecha.toLocaleTimeString();

var fechaTime = fechaString + ' ' + timeString;
...
window.onload = () => {
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            ...
            signinBtn.innerText = "Minha Conta";
            localStorage.clear()
...
categoryForm.onsubmit = () => {
    ...
    if (Boolean(selectedCategory) === true) {
        categoryName.innerText = `Categoria: ${selectedCategory.value}`;
...
function show_features(){
    ...
    firebase.database().ref('/ads').limitToLast(8).on('child_added', (snapshot) => {
...
function filter(category) {
    ...
    browseCategories.innerHTML = `Navegar por categorias &nbsp; / &nbsp; ${category}`;
    ...
    if (picks.innerHTML.trim().length === 0) {
        picks.innerHTML = `<div style="text-align:center; margin:50px 0;"><h2>Oops! Não há nada para mostrar :(</h2><img src="./images/noresults.png" alt=""></div>`;
    }
}
...
myAds.addEventListener('click', () => {
    ...
    picks.innerHTML = '';
    categoryHeading.innerText = 'Meus Anúncios';
...
function showAdDetails(figure) {
    ...
    firebase.database().ref(`/ads/${adUid}`).on('value', (snapshot) => {
        ...
        if (adUid === (snapshot.key)) {
            ...
            price.innerHTML = `€ ${adDetails.price}`;
...
//let adsUid = figure.childNodes[1];
console.log("M"+ figure);

deleteAdBtn.onclick = () => {
    var figure = document.getElementById("figure");

     var result = confirm("Com certeza deseja excluir?");
     if (result) {
        console.log(figure);
